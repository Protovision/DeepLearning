...
TinyImageNet = function( sourceDir )
	...
	if File.exists( sourceDir .. "/TinyImageNetClasses.table" ) then
		tin._.classes = torch.load( sourceDir .. "/TinyImageNetClasses.table" )
	else
		local fs, line, i
		tin._.classes = {}
		...
		fs:close()
		torch.save( sourceDir .. "/TinyImageNetClasses.table", tin._.classes )
	end
	...
	tin.TrainingSet = function( self )
		...
		if File.exists( trainPath .. "/TinyImageNetTrainingImages.Tensor" ) and 
		File.exists( trainPath .. "/TinyImageNetTrainingLabels.Tensor" ) then
			ts.inputs  = torch.load( trainPath .. "/TinyImageNetTrainingImages.Tensor" )
			ts.targets = torch.load( trainPath .. "/TinyImageNetTrainingLabels.Tensor" )
		else
			...
			torch.save( trainPath .. "/TinyImageNetTrainingImages.Tensor", ts.inputs )
			torch.save( trainPath .. "/TinyImageNetTrainingLabels.Tensor", ts.targets )
		end
	...
	tin.ValidationSet = function ( self )
		...
		if File.exists( valPath .. "/TinyImageNetValidationImages.Tensor" ) and
		File.exists( valPath .. "/TinyImageNetValidationLabels.Tensor" ) then
			vs.inputs = torch.load( valPath .. "/TinyImageNetValidationImages.Tensor" )
			vs.targets = torch.load( valPath .. "/TinyImageNetValidationLabels.Tensor" )
		else
			...
			fs:close()
			torch.save( valPath .. "/TinyImageNetValidationImages.Tensor", vs.inputs )
			torch.save( valPath .. "/TinyImageNetValidationLabels.Tensor", vs.targets )
		end
...
return TinyImageNet( "tiny-imagenet-200" )
